...
<head>
    ...
    <title>Acadsuite FOSE - Gestión de Documentos</title>
...
<body>
    ...
    <main id="main" class="main">
        <div class="pagetitle">
            <h1>Gestión de Documentos <?php echo $ano; ?></h1>
            <nav>
                <ol class="breadcrumb">
        ...
        <table class="table">
            ...
            <tbody>
                <?php while ($row = $result->fetch_assoc()): ?>
                    <tr>
                        <td><?= htmlspecialchars($row['iddoc']) ?></td>
                        ...
                        <td class="d-flex justify-content-end">
                            <button type="button" class="btn btn-primary edit-btn mx-1"
                                data-id="<?= $row['iddoc'] ?>"
                                data-idcuenta="<?= htmlspecialchars($row['idcuenta']) ?>"
                                data-documento="<?= htmlspecialchars($row['documento']) ?>"
                                data-naturaleza="<?= htmlspecialchars($row['naturaleza']) ?>"
                                data-bs-toggle="modal" data-bs-target="#editDocumentoModal">Editar</button>
                            <button type="button" class="btn btn-danger delete-btn mx-1" data-id="<?= $row['iddoc'] ?>">Eliminar</button>
                        </td>
                    </tr>
                <?php endwhile; ?>
            </tbody>
