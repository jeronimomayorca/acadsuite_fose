...
<head>
    ...
    <title>Acadsuite FOSE - Gestión de CGC Grupos</title>
...
<body>
    ...
    <main id="main" class="main">
        <div class="pagetitle">
            <h1>Gestión de CGC Grupos <?php echo $ano; ?></h1>
            <nav>
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="../../menu.php">Inicio</a></li>
                    <li class="breadcrumb-item active">CGC Grupos</li>
                </ol>
            </nav>
            <nav>
        ...
        <table class="table">
            ...
            <tbody>
                <?php while ($row = $result->fetch_assoc()): ?>
                    <tr>
                        <td><?= htmlspecialchars($row['idgrupo']) ?></td>
                        ...
                        <td class="d-flex justify-content-end">
                            <button type="button" class="btn btn-primary edit-btn mx-1"
                                ...
                                data-idclase="<?= htmlspecialchars($row['idclase']) ?>"
                                data-nivel="<?= htmlspecialchars($row['nivel']) ?>"
                                data-naturalezacuenta="<?= htmlspecialchars($row['naturalezacuenta']) ?>"
                                data-bs-toggle="modal" data-bs-target="#editGrupoModal">Editar</button>
                            <button type="button" class="btn btn-danger delete-btn mx-1" data-id="<?= $row['idgrupo'] ?>">Eliminar</button>
                        </td>
                    </tr>
                <?php endwhile; ?>
            </tbody>
