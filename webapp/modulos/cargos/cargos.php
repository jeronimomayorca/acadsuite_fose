...
<head>
    ...
    <title>Acadsuite FOSE - Gestión de Cargos</title>
...
<body>
    ...
    <main id="main" class="main">
        ...
        <div class="pagetitle">
            <h1>Gestión de Cargos <?php echo $ano ?></h1>
            <nav>
                <ol class="breadcrumb">
        ...
        <table class="table">
            ...
            <tbody>
                <?php while ($row = $result->fetch_assoc()): ?>
                    <tr>
                        <td><?= htmlspecialchars($row['idcargo']) ?></td>
                        <td><?= htmlspecialchars($row['cargo']) ?></td>
                        <td class="d-flex justify-content-end">
                            <button type="button" class="btn btn-primary edit-btn mx-1"
                                data-id="<?= $row['idcargo'] ?>"
                                data-cargo="<?= htmlspecialchars($row['cargo']) ?>"
                                data-bs-toggle="modal" data-bs-target="#editCargoModal">Editar</button>
                            <button type="button" class="btn btn-danger delete-btn mx-1" data-id="<?= $row['idcargo'] ?>">Eliminar</button>
                        </td>
                    </tr>
                <?php endwhile; ?>
            </tbody>
