...
<head>
    ...
    <title>Acadsuite FOSE - Gestión de Nombre Cuentas</title>
...
<body>
    ...
    <main id="main" class="main">
        <div class="pagetitle">
            <h1>Gestión de Nombre Cuentas <?php echo $ano; ?></h1>
            <nav>
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="../../menu.php">Inicio</a></li>
                    <li class="breadcrumb-item active">Nombre Cuentas</li>
                </ol>
            </nav>
            <nav>
            ...
            <tbody>
                <?php while ($row = $result->fetch_assoc()): ?>
                    <tr>
                        ...
                        <td><?= htmlspecialchars($row['nombrecuenta']) ?></td>
                        ...
                        <td class="d-flex justify-content-end">
                            <button type="button" class="btn btn-primary edit-btn mx-1"
                                data-id="<?= $row['idnombrecuenta'] ?>"
                                data-orden="<?= htmlspecialchars($row['orden']) ?>"
                                data-nombrecuenta="<?= htmlspecialchars($row['nombrecuenta']) ?>"
                                data-tipocuenta="<?= htmlspecialchars($row['tipocuenta']) ?>"
                                data-modalidad="<?= htmlspecialchars($row['modalidad']) ?>"
                                data-idauxiliar="<?= htmlspecialchars($row['idauxiliar']) ?>"
                                data-bs-toggle="modal" data-bs-target="#editNombreCuentaModal">Editar</button>
                            <button type="button" class="btn btn-danger delete-btn mx-1" data-id="<?= $row['idnombrecuenta'] ?>">Eliminar</button>
                        </td>
                    </tr>
                <?php endwhile; ?>
            </tbody>
