...
<head>
    ...
    <title>Acadsuite FOSE - Gestión de Subfuentes</title>
...
<body>
    ...
    <main id="main" class="main">
        <div class="pagetitle">
            <h1>Gestión de Subfuentes <?php echo $ano; ?></h1>
            <nav>
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="../../menu.php">Inicio</a></li>
                    <li class="breadcrumb-item active">Subfuentes</li>
                </ol>
            </nav>
            <nav>
            ...
            <tbody>
                <?php while ($row = $result->fetch_assoc()): ?>
                    <tr>
                        ...
                        <td><?= htmlspecialchars($row['distribucionniv']) ?></td>
                        <td class="d-flex justify-content-end">
                            <button type="button" class="btn btn-primary edit-btn mx-1"
                                data-id="<?= $row['idsubfuente'] ?>"
                                data-idfuente="<?= htmlspecialchars($row['idfuente']) ?>"
                                data-subfuente="<?= htmlspecialchars($row['subfuente']) ?>"
                                data-distribucionniv="<?= htmlspecialchars($row['distribucionniv']) ?>"
                                data-bs-toggle="modal" data-bs-target="#editSubfuenteModal">Editar</button>
                            <button type="button" class="btn btn-danger delete-btn mx-1"
                                data-id="<?= $row['idsubfuente'] ?>">Eliminar</button>
                        </td>
                    </tr>
                <?php endwhile; ?>
            </tbody>
